...
public class CustomerMain extends Register {
	...
	void trackOrder(Customer c) {
		...
		if (!c.getOrderStatus().containsKey(orderId)) {
			System.out.println("Invalid order id");
			return;
		}
		System.out.println(c.getOrderStatus().get(orderId));
		Input.keepScreen();
	...
	void viewRecentOrders(Customer c) {
		...
		for(String str:c.getCompletedOrders()) {
	...
	public boolean signup() {
		...
		if (super.signup()) {
			if (super.newPassword()) {
				...
				if (!HotelDB.isServiceable(pincode)) {
					...
					if (input == -1)
						return false;
					if (input == 1) {
						CustomerDB.addCustomer(new Customer(getName(), getPhoneNo(), getMailId(), getAddress(),
