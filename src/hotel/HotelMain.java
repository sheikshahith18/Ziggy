...
public class HotelMain extends Register {
	...
	public boolean signup() {
		...
		if (flag) {
			if (super.newPassword()) {
				...
				HotelDB.addHotel(new Hotel(getName(), ownerName, getAddress(), getPincode(), getPhoneNo(), getMailId(),
	...
	public void viewOrders(Hotel h) {
		...
		if (!h.isOrderAvailable(h.getPendingOrders().get(orderId))) {
			...
			CustomerDB.getCustomer(h.getPendingOrders().get(orderId).getCustMail()).addCompletedOrder(str);
			System.out.println("insufficient products");
			CustomerDB.getCustomer(h.getPendingOrders().get(orderId).getCustMail()).getUPI()
		...
		if (option == 1) {
			...
			CustomerDB.getCustomer(h.getPendingOrders().get(orderId).getCustMail()).getPendingOrder().remove(orderId);
